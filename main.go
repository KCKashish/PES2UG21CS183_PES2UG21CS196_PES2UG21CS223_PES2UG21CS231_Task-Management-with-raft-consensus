...
func InitDB() *sql.DB {
	db, err := sql.Open("mysql", "root:ajja7777@tcp(127.0.0.1:3306)/task_manager")
	if err != nil {
...
func UpdateTask(db *sql.DB, task Task) error {
    ...
    res, err := db.Exec("UPDATE tasks SET description = ?, status = ? WHERE id = ?", task.Description, task.Status, task.ID)
    if err != nil {
        return err
    }
...
func handleTasks(db *sql.DB,w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPut: // Change to handle PUT requests
        var task Task
...
func addTasksToDatabase(db *sql.DB) {
	...
	tasks := []Task{
		...
		{Description: "Task 14", Status: "Pending"},
		{Description: "Task 31", Status: "Completed"},
...
func main() {
	...
	// Start HTTP server
	fmt.Println("Server is running on port 8503")
	http.ListenAndServe(":8503", nil)
}